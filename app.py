...
from planit_client import fetch_applications, geocode_postcode
...
def signup():
    ...
    if request.method == "POST":
        ...
        # so a geocoding hiccup should just mean no map, never a broken
        # results page. fetch_applications() geocodes again internally for
        # its own PlanIt query — a little redundant, but keeps this map
        # concern fully decoupled from the search/escalation logic.
        map_center = None
        try:
            map_lat, map_lng = geocode_postcode(postcode)
            map_center = {"lat": map_lat, "lng": map_lng}
        except requests_lib.exceptions.RequestException:
            logger.exception("Failed to geocode postcode for results-page map — page still works without it")
...
def preview(subscriber_id):
    ...
    try:
        subject, body, _html_body, new_records = build_digest_for_subscriber(subscriber, preview=True)
    except requests_lib.exceptions.RequestException:
    ...
    approved_records = [r for r in new_records if r.get("_stage") == "approved"]
    other_records = [r for r in new_records if r.get("_stage") != "approved"]
    return render_template(
        "preview.html", subscriber=subscriber, subject=subject, body=body, records=new_records,
        approved_records=approved_records, other_records=other_records,
